...
speed = 1 / float(open("data/speed.txt", "r").read())
makeSmaller = float(open("data/compress.txt", "r").read()) + 2
while flag:
	with mss.mss() as sct:
...
for i in range(len(images)):
	...
	images[i] = temp.resize((floor(temp.width / (makeSmaller / 2)), floor(temp.height / (makeSmaller / 2))))
flag2 = True
i = 0
while flag2:
	try:
